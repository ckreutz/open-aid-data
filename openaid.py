...
def show_donors():
    ...
    # filter out donors mit fehlerseiten
...
def show_recipient_year(country,year):
    ...
    if year > g.year:
        totalYear = query_db_one("select sum(value_usd) as main_value from iati where recipient_country = (select iati_country from countries where crs_code = {1}) and YEAR(value_date) = {0} and transaction_type_code IN ('D', 'E')".format(year, country))
    else:
    ...
    iati_entries = []
    for row in query_db("select sector as sectorname, count(id) as transactions, id, sum(value_usd) as main_value from iati where recipient_country = (select iati_country from countries where crs_code = {0}) and YEAR(value_date) = {1} and sector != '' and value_usd > 0 and transaction_type_code IN ('D', 'E') group by sector order by main_value DESC limit 30".format(country, year)):
            d = dict(row.items())
            if year > g.year:
